...
static void search( GtkWidget *w,gpointer data )
{
    ...
    //GtkWidget *edit_btn=((GtkWidget**)data)[2];
...
static void edit( GtkWidget *w,gpointer data )
{
    ...
    {
		Show_message(window1, GTK_MESSAGE_INFO, "Cách dùng:", "Phải tìm kiếm trước khi sửa.");
		return;
    }
    //search(NULL,data);
...
void tra_tu(GtkWidget widget, gpointer window)
{
    ...
    GtkWidget *btn1,*window1,*label,*entry,*btn2,*btn3,*label3;
    ...
    gtk_fixed_put(GTK_FIXED(fixed), btn1, 400, 15);
    gtk_widget_set_size_request(btn1, 80, 30);

    /*label_print = gtk_label_new(NULL);
    gtk_label_set_use_markup (GTK_LABEL (label_print), TRUE);
    gtk_fixed_put(GTK_FIXED(fixed), label_print, 50, 50);*/

    GtkWidget *label2=gtk_label_new("Nghĩa :");
    gtk_table_attach(GTK_TABLE(table), label2, 0, 1, 1, 2,GTK_FILL | GTK_SHRINK, GTK_FILL | GTK_SHRINK, 15, 15);

    textView = gtk_text_view_new();
    gtk_widget_set_size_request(textView, 300, 500);
...
void them_tu(GtkWidget widget, gpointer window)
{
    ...
    gtk_window_set_default_size(GTK_WINDOW(window1), 600, 410);
    gtk_window_set_position(GTK_WINDOW(window1), GTK_WIN_POS_CENTER);
    ...
    g_signal_connect(G_OBJECT(btn1), "clicked",G_CALLBACK(add), window1);
...
void xoa_tu(GtkWidget widget, gpointer window)
{
    ...
    gtk_window_set_default_size(GTK_WINDOW(window1), 600, 150);
    gtk_window_set_position(GTK_WINDOW(window1), GTK_WIN_POS_CENTER);
...
void masturbation(GtkWidget widget, gpointer window)
{
    about_dialog = gtk_about_dialog_new();
	gtk_about_dialog_set_program_name(GTK_ABOUT_DIALOG(about_dialog), "Từ điển Anh-Việt");
	gtk_about_dialog_set_version(GTK_ABOUT_DIALOG(about_dialog), "6.9");
	gtk_about_dialog_set_copyright(GTK_ABOUT_DIALOG(about_dialog), "@Masturbation Group");
	gtk_about_dialog_set_website(GTK_ABOUT_DIALOG(about_dialog),
	                             "https://drive.google.com/drive/folders/0B7ihu5NRRLO1Y0FQOU5nSjdvSW8?usp=sharing");
	gtk_about_dialog_set_website_label(GTK_ABOUT_DIALOG(about_dialog),
                                "https://drive.google.com/drive/folders/0B7ihu5NRRLO1Y0FQOU5nSjdvSW8?usp=sharing");
	gtk_about_dialog_set_comments(GTK_ABOUT_DIALOG(about_dialog), "Chương trình từ điển sử dụng cấu trúc B-tree");
...
int main(int argc, char *argv[])
{
    ...
    gtk_fixed_put(GTK_FIXED(fixed), btn1, 550, 20);
    ...
    gtk_fixed_put(GTK_FIXED(fixed), btn2, 550, 90);
    ...
    gtk_fixed_put(GTK_FIXED(fixed), btn3, 550, 160);
    ...
    btn4 = gtk_button_new_with_label("Masturbation");
    gtk_fixed_put(GTK_FIXED(fixed), btn4, 550, 230);
    gtk_widget_set_size_request(btn4, 120, 50);

    g_signal_connect(G_OBJECT(btn4), "clicked",G_CALLBACK(masturbation), NULL);

    btn5 = gtk_button_new_with_label("Thoát");
    gtk_fixed_put(GTK_FIXED(fixed), btn5, 550, 300);
    gtk_widget_set_size_request(btn5, 120, 50);
    gtk_widget_set_tooltip_text(btn5, "Exit");

    g_signal_connect(G_OBJECT(btn5), "clicked",G_CALLBACK(gtk_main_quit), G_OBJECT(window));
